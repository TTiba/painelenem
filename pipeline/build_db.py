#!/usr/bin/env python3
"""
Pipeline ENEM 2025 — agrega microdados em SQLite para a plataforma.

Níveis: BR > UF > MUN (município) > ESC (escola).
Métricas: notas médias por área + redação (e 5 competências) e acerto por item.

Só participantes com CO_ESCOLA preenchido (concluintes do ano) entram — é o
recorte que permite atribuir desempenho a uma escola.

Uso:  .venv/bin/python pipeline/build_db.py
"""
import os
import sqlite3
import time
...
DADOS = os.path.join(os.path.dirname(BASE), "microdados_enem_2025", "DADOS")
DB_OUT = os.path.join(BASE, "data", "enem2025.sqlite")

RESULTADOS = os.path.join(DADOS, "RESULTADOS_2025.csv")
ITENS = os.path.join(DADOS, "ITENS_PROVA_2025.csv")
...
# metadados por item (1 linha por CO_ITEM; gabarito/params são idênticos
# entre cadernos). habilidade_custom fica NULL até recebermos o mapa externo.
con.execute("""
...
# ---------------------------------------------------------------- resultados
log("Lendo RESULTADOS_2025.csv (2,1 GB — só concluintes com escola)…")
cols = """
...
log(f"Concluintes com escola: {n:,}")

# chaves de agregação (chave = string única por entidade em cada nível)
# rede: PUB = federal/estadual/municipal, PRIV = privada
con.execute("""
...
def resumo_select(nivel, chave, nome, uf, grupo=None, rede_val="T", where=""):
...
for rv in ("T", "PUB", "PRIV"):
    ...
    partes_resumo += [
        ...
        resumo_select('UF', 'k_uf', 'any_value(k_uf)', 'any_value(k_uf)',
                      'k_uf', rede_val=rv, where=w),
        resumo_select('MUN', 'k_mun', "any_value(NO_MUNICIPIO_ESC)",
                      'any_value(k_uf)', 'k_mun', rede_val=rv, where=w),
    ]
con.execute("CREATE TABLE agg_resumo AS " + " UNION ALL ".join(partes_resumo))

# catálogo de escolas (para o seletor): código, município, dependência, n
con.execute("""
    ...
    SELECT k_esc AS chave,
           ...
           count(*) AS n_participantes,
           CAST(NULL AS VARCHAR) AS nome   -- preencher via Censo Escolar depois
    FROM res_k GROUP BY k_esc
""")

# ---------------------------------------------------------------- itens longos
log("Explodindo respostas item a item (~300M comparações)…")
partes = []
...
# passo 1: escolas (uma escola pertence a uma única rede -> rede 'T')
con.execute(f"""
...
""")
log("  escolas ok; pré-agregando município × rede…")
# passo 2: município × rede (base compacta para os rollups MUN/UF/BR)
con.execute(f"""
...
""")
con.execute("""
    CREATE TABLE agg_item AS
    SELECT * FROM agg_item_esc
    UNION ALL
    SELECT 'MUN', k_mun, rede, CO_ITEM, n, acertos, esperado FROM mun_item
    UNION ALL
...
""")
con.execute("DROP TABLE agg_item_esc; DROP TABLE mun_item;")
