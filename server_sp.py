#!/usr/bin/env python3
"""API + estáticos do site paralelo (São Paulo) — porta 8092 por padrão."""
import json
import os
import sqlite3
import sys
...
def api(path, qs):
    """Mesma API do server.py, mas /api/ufs devolve só SP e /api/municipios
    ignora o parâmetro uf (sempre SP)."""
    rede = rede_de(qs)

    if path == "/api/ufs":
        # devolve só SP (compat com o cliente do site principal)
        return q("""SELECT chave, nome, n_participantes, media_geral,
                           media_red, media_lc, media_ch, media_cn, media_mt
                    FROM agg_resumo
                    WHERE nivel='UF' AND chave='SP' AND rede=?""", (rede,))

    if path == "/api/municipios":
        # UF é sempre SP no site paralelo
        return q("""SELECT chave, nome, n_participantes, media_geral,
                           media_red, media_lc, media_ch, media_cn, media_mt
                    FROM agg_resumo WHERE nivel='MUN' AND uf='SP' AND rede=?
                    ORDER BY nome""", (rede,))
    ...
    if path == "/api/escolas":
        ...
        rows = q(f"""SELECT e.chave, e.nome, e.dependencia, e.n_participantes,
                    ...
                    JOIN agg_resumo r ON r.nivel='ESC' AND r.chave=e.chave
                                     AND r.rede='T'
                    WHERE e.co_municipio=? AND e.uf='SP' {filtro_rede}
    ...
    if path == "/api/itens":
        ...
        uf = "SP"  # sempre SP como referência regional
...
if __name__ == "__main__":
    porta = int(sys.argv[1] if len(sys.argv) > 1 else os.environ.get("PORT", 8092))
    print(f"Painel ENEM · SP em http://localhost:{porta}", flush=True)
